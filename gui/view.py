...
class View:
    ...
    def __init__(self, root, api_client):
        ...
        # Intensity
        ttk.Label(scales_frm, text='Intensity:').grid(column=0, row=0, sticky="ew")
        ttk.Scale(scales_frm, from_=0, to=100, variable=self.intensity_v).grid(column=1, row=0)
        ttk.Label(scales_frm, textvariable=self.intensity_strv, width=4).grid(column=2, row=0)

        # Duration
        ttk.Label(scales_frm, text='Duration (ms):').grid(column=0, row=1, sticky="ew")
        ttk.Scale(scales_frm, from_=300, to=3000, variable=self.duration_v).grid(column=1, row=1)
        ttk.Label(scales_frm, textvariable=self.duration_strv, width=4).grid(column=2, row=1)

        # Type frame
        type_frm = ttk.Frame(mainframe, padding=10)
        type_frm.grid(column=0, row=3)
        ttk.Radiobutton(type_frm, text='Stop', variable=self.type_v, value='Stop').grid(column=0, row=0, padx=5)
        ttk.Radiobutton(type_frm, text='Shock', variable=self.type_v, value='Shock').grid(column=1, row=0, padx=5)
        ttk.Radiobutton(type_frm, text='Vibrate', variable=self.type_v, value='Vibrate').grid(column=2, row=0, padx=5)

        # Footer frame
        footer_frm=ttk.Frame(mainframe, padding=10)
        footer_frm.grid(column=0, row=4)
        ttk.Button(footer_frm, text='Send', command=self.send).grid(column=0, row=0, sticky='e')
        ttk.Button(footer_frm, text='Quit', command=root.destroy).grid(column=1, row=0, sticky='w')
